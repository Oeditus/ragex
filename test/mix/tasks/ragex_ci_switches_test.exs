defmodule Mix.Tasks.Ragex.CiSwitchesTest do
  ...
  describe "MetaCredoBridge db_check? and user_check?" do
    ...
    test "filter_checks filters checks based on no_db and no_user" do
      ...
      refute Enum.any?(no_db_filtered, fn {mod, _} -> mod == MetaCredo.Check.Warning.NPlusOneQuery end)

      no_user_filtered = MetaCredoBridge.filter_checks(checks, no_user: true)
      assert length(no_user_filtered) == 2
      refute Enum.any?(no_user_filtered, fn {mod, _} -> mod == MetaCredo.Check.Security.XSSVulnerability end)
  ...
  describe "Runner.filter_results_by_switches/2" do
    test "suppresses db and user input issues from analysis results" do
      ...
      no_db_results = Ragex.Analysis.Runner.filter_results_by_switches(results, %{no_db: true, no_user: false})
      ...
      no_user_results = Ragex.Analysis.Runner.filter_results_by_switches(results, %{no_db: false, no_user: true})
      ...
      both_results = Ragex.Analysis.Runner.filter_results_by_switches(results, %{no_db: true, no_user: true})
